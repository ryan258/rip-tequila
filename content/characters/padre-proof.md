---
title: "Padre Proof"
weight: 30
role: "The Priest"
accent: "gold"
card_bio: "Forgives no one. Raises the bar."
prop: "The Ledger"
prop_desc: "A black leather-bound ledger titled 'Still Owed to the Bartender'."
prop_icon: "barrel"
pour_title: "RIP Añejo"
pour_desc: "Rich, deep amber, double-distilled to carry the weight of a final confession."
pour_line: "YOUR SINS ARE BETWEEN YOU AND THE RECEIPT."
pour_href: "/the-lineup/rip-anejo/"
pour_image: "images/bottles/anejo-transparent.png"
location_image: "images/cantina/the-absolution-booth.webp"
icon: "cross"
line: "I bless this mess."
highlights:
  - title: "Unbureaucratic Absolution"
    text: "Grants generous sign-offs on late-night vows."
    icon: "cross"
  - title: "Theological Tolerance"
    text: "Has heard every human folly and found them all entertaining."
    icon: "skull"
  - title: "Midnight Confessions"
    text: "The velvet corner booth is a sanctuary for secrets."
    icon: "monument"
  - title: "Blessing the Mess"
    text: "Will bless any glass and declare it a sacrament."
    icon: "glass"
speaks:
  - title: "No Judgment past 11"
    text: "Questionable choices are simply research for the ledger."
    icon: "cross"
  - title: "Order Another Round"
    text: "It is a sign of faith in the morning after."
    icon: "barrel"
  - title: "Write It on the Tab"
    text: "The ledger remembers what the conscience forgets."
    icon: "bell"
  - title: "Absolution is Free"
    text: "But the tequila is charged by the shot."
    icon: "skull"
seo_title: "Padre Proof — The Blesser of Bad Decisions | RIP Tequila"
seo_description: "Padre Proof presides over questionable choices, signs off on midnight vows, and provides absolution for bar tabs at La Cantina del Último Trago."
---

**Padre Proof** is the resident spiritual advisor for individuals whose decision-making skills diminish rapidly after 11:00 PM.

Stationed in the velvet corner booth, he offers full, unbureaucratic absolution for impulsive toasts, sudden romantic declarations, and questionable karaoke song selections. His paperwork is pristine; his theological tolerance is infinite.

He carries a worn, black leather-bound volume that patrons assume is a liturgical breviary. In reality, it contains tasting notes, cocktail recipes, and a single handwritten ledger titled *"Still Owed to the Bartender."* He has never once judged a soul who sat in his booth — mostly because he has heard every possible variation of human folly and found them all thoroughly entertaining.

If you’re wondering whether ordering that extra round of Añejo is a sound life choice, consult Padre Proof. He’ll make the sign of the cross, bless the glass, and tell you it’s a sacrament.

> *"Blessed are the almost responsible."*

## What he pours

**Padre Proof Ceremonial Añejo**, a rich, reverent pour formulated specifically for promises made past midnight.

> *"I bless this mess."*
>
> *"Your sins are between you, the lime, and the receipt."*
