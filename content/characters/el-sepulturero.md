---
title: "El Sepulturero"
weight: 40
role: "The Cabinet"
accent: "green"
card_bio: "Here lies your judgment."
prop: "Iron Spade"
prop_desc: "Used to bury the collateral damage of memorable nights."
prop_line: "He digs where the night needs burying, and he never asks what goes in."
prop_icon: "shovel"
pour_title: "RIP Extra Añejo"
pour_desc: "Aged 36 months in deep oak darkness, complex with rich chocolate and smoke."
pour_line: "SOME NIGHTS DESERVE A SHOVEL."
pour_href: "/the-lineup/rip-extra-anejo/"
pour_image: "images/bottles/extra-anejo-transparent.png"
location_image: "images/cantina/the-back-office.webp"
icon: "shovel"
line: "Here lies your judgment."
lead: "He works the quiet hours between closing time and whatever comes next — spade in one hand, Manifest in the other."
highlights:
  - title: "Caretaker of the Morning"
    text: "Manages the quiet hours and inters what is left behind."
    icon: "monument"
  - title: "Burying the Mistakes"
    text: "Buries lost arguments, bad texts, and abandoned dignity."
    icon: "shovel"
  - title: "Dry Wit"
    text: "Possesses a razor-sharp humor and zero patience for vanity."
    icon: "skull"
  - title: "The Manifest Keeper"
    text: "Tracks every soul who checked in and forgot to check out."
    icon: "eye"
speaks:
  - title: "Leave the Pride"
    text: "Wounded pride is interred with full military honors."
    icon: "shovel"
  - title: "Keep the Story"
    text: "The details are worth more than the argument."
    icon: "rose"
  - title: "No Morning Regrets"
    text: "A bad choice makes a great headstone."
    icon: "monument"
  - title: "Six Feet of Silence"
    text: "Some arguments are best settled in deep clay."
    icon: "cross"
seo_title: "El Sepulturero — The Caretaker of the Cantina | RIP Tequila"
seo_description: "El Sepulturero manages the morning after at La Cantina del Último Trago. Here lies your judgment. Meet the deadpan gravedigger behind RIP Extra Añejo."
---

**El Sepulturero** handles the quiet hours between closing time and whatever comes next.

When the toasts have run out, the brass bell has quieted, and the patrons have stumbled out into the desert breeze, he walks the grounds with a heavy iron spade and a ledger known simply as *"The Manifest."* 

He does not bury people — he buries the collateral damage of a memorable evening. Lost arguments, ill-advised text messages sent at 2:43 AM, forgotten jackets, and abandoned dignity are all interred with solemn dignity and marked with a discreet stone.

He possesses a dry, unflinching wit and an encyclopedic record of every tab ever run in the cantina's history. He doesn't judge you for your mistakes; he simply categorizes them.

> *"Leave the judgment. Take the story."*

## What he pours

**RIP Extra Añejo**, aged 36 months in deep darkness. For extraordinary occasions, he unearths **Gravedigger's Reserve.**

>

